...
select u.id from subscription s 
inner join users_has_subscription uhs on s.id_subscription = uhs.subscription_id 
inner join users u on u.id = uhs.user_id 
where s.id_subscription = (select s.id_subscription where datediff(adddate(date(s.date_of_activate), s.duration),date('2022-10-27')) > 0);#не работает